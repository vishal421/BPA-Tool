...
app.post("/api/upload-file", upload.single("file"), async (req, res) => {
  const { upload_url, skip_gzip } = req.body;
  if (!upload_url) return res.status(400).json({ error: "upload_url is required" });
  if (!req.file) return res.status(400).json({ error: "No file received" });

  try {
    const payload = skip_gzip === "true" ? req.file.buffer : zlib.gzipSync(req.file.buffer);
    ...
    return res.json({ ok: true, status: result.statusCode, mode: skip_gzip === "true" ? "raw (test mode)" : "gzipped" });
  } catch (err) {
