...
app.post("/api/upload-file", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (!putResp.ok) {
      return res.status(putResp.status).json({ error: "Upload to presigned URL failed", details: text });
    }
    return res.json({ ok: true, status: putResp.status });
  } catch (err) {
...
app.post("/api/upload-from-url", async (req, res) => {
  ...
  try {
    ...
    return res.json({ ok: true, status: putResp.status, bytes: arrayBuf.byteLength });
  } catch (err) {
